...
Use npm Trusted Publishing and provenance for Forge package releases. Forge does not introduce a second signed release manifest. Each published package embeds the compatible APM release metadata and verifies downloaded APM artifacts with SHA-256 before invoking a documented installer directly.
...
Node.js becomes an explicit prerequisite and the native archive/signing/notarisation pipeline is removed. npm, pnpm, and Yarn users can bootstrap the same package, while Forge uses npm for its own global installation. The stdio MCP server becomes a public, schema- and fixture-tested cross-repository contract. CI tests Node.js 22 and 24 on Windows, macOS, and Linux.
